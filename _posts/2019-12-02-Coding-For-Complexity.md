...
category: Tech
tags: [xstate, State Machines, State Diagrams]
---
...
Without a structured approach to iterating the states our code can be in it can be very difficult to make sure all your bases are covered. I'd argue that most, if not all developers have stumbled upon routines consisting of questionably named flag variables used in convoluted combinations whose author is either long gone or too promoted to ask anymore.
...
Whether you call them bugs, features, or off nominal scenarios, statecharts can help developers identify edge cases and scale for complexity.
...
xstate implements the statechart model with the following features:
...
It also describes how it manages those state machines as actors. The actor model is a mathematical model of concurrent computation that treats "actors" as the universal primitives of a system, with each able to perform three core functions:
...
xstate considers each state machine to be an actor which encapsulates its own logic and exchanges information with other state machines through events. In this way, an active state machine can be thought of similarly to a process running on your device, unable to access data from other services directly but able to share input and output between them.
...
1. **onPress**: A prop for controlling what happens when it's clicked.
2. **enabled**: A prop for manually enabling or disabling the button.
3. **isLoading**: A prop for controlling whether the button should display a loading state.
...
Now let's compare this to an explicit state machine model using xstate:
...
The `useMachine` API for React components interprets the given state machine and starts it as a service:
...
The ability to visualize the states of a UI component can benefit the entire team. 
...
As actors, each statechart can spawn and manage its own child services. We can create a `BankApplicationManagerMachine` to handle multiple applications:
  ...
  on: {
    OPEN_APPLICATION: {
      ...
      actions: assign((context, event) => {
        ...
        application = spawn(BankApplicationMachine, uuid());
...
2. Applications services and their data is stored by the application manager and can be persisted and restored without any external data mechanism.
...
4. Business logic and its testing is independent of UI
...
* It's a different programming model, everyone is used to standard if/else conditional statements and *a boolean is much simpler!* is a common counterargument to the boilerplate of state machine implementations.

The argument and examples above make the case that the benefits of using state machines largely outweigh these potential concerns and ultimately present a compelling case for managing the shared, mutable states of our applications.
